...
- **Updating the server:** `houston update [vX.Y.Z]`, with the human's OK, updates the server to the latest release (or that one) and follows it to the result: exit 0 on GO, 1 if it failed (and whether it went back to the old version). It's refused while a deploy, restore or backup is running; wait and try again. Mission Control restarts on the way, so for a minute or two it doesn't answer. `houston status` says "(updating to vX.Y.Z)" meanwhile. `houston update --check` has the server ask GitHub for the latest release now (it's otherwise checked every 6 hours).
