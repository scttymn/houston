...
class Settings::GeneralControllerTest < ActionDispatch::IntegrationTest
  ...
  test "settings is one page: the menu and its sections" do
    ...
    assert_equal [ [ "API tokens", "#tokens" ], [ "Cloudflare", "#cloudflare" ], [ "Storage", "#storage" ], [ "Time zone", "#time-zone" ] ], links
    assert_equal %w[tokens cloudflare storage time-zone], css_select(".settings__body > section").map { |s| s["id"] }
