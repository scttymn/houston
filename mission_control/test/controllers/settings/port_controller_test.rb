...
# Settings › Security (security fixes, H3): what port 3000 is bound to
...
class Settings::PortControllerTest < ActionDispatch::IntegrationTest
  ...
  test "it says what port 3000 is bound to, and offers the other" do
    ...
    assert_select "section#security" do
      assert_select ".panel__title", "Security"
      assert_select "#port .settings-subhead", "PORT 3000"
    ...
    assert_select "section#security #port .panel__row", /CLOSED\s+only on this server \(127\.0\.0\.1\)/
    assert_select "section#security #port input[name=open][value='1']", 1

    Rails.cache.clear
    use_fake_docker(docker("100.64.1.2")) { get settings_path }
    assert_select "section#security #port .panel__row", /OPEN\s+on 100\.64\.1\.2/

    Rails.cache.clear
    use_fake_docker(FakeDocker.new { failure("permission denied") }) { get settings_path }
    assert_select "section#security #port .panel__row", /can't tell/i
    assert_select "section#security #port form", 0
  end
  ...
  test "closing it from port 3000 goes on to admin.<base> first" do
    ...
    assert_redirected_to "https://admin.svnmns.com/settings#security"
    assert_equal 1, @runs
