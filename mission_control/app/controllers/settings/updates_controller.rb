# Settings › Houston: check GitHub for a newer Houston, update this server
...
class Settings::UpdatesController < ApplicationController
  ...
  def check
    if (found = LatestRelease.check_and_say)
      redirect_to settings_path(anchor: "houston"), flash: { toast: { "kind" => "go", "text" => found } }
    else
      redirect_to settings_path(anchor: "houston"), flash: { toast: { "kind" => "nogo", "text" => GITHUB_DOWN } }
    end
  end
  ...
  def create
    ...
    redirect_to settings_path(anchor: "houston"), flash: { toast: { "kind" => "nogo", "text" => "No update: Houston #{e.message}." } }
  end
end
