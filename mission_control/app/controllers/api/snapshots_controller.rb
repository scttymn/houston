...
# per deploy: a retried POST gets the same run.
class Api::SnapshotsController < Api::BaseController
  before_action :set_deploy

  def create
    run = BackupRun.request!(@deploy.project, reason: "deploy", deploy_number: @deploy.number)
  ...
  def show
    run = @deploy.project.backup_runs.find_by(reason: "deploy", deploy_number: @deploy.number)
