...
# verify are counted per client address, and checked before the body is
# read, so junk from one address can't block another's real pushes.
...
class WebhooksController < ActionController::API
  ...
  def create
    return head :too_many_requests if count("unverified", request.remote_ip) >= UNVERIFIED
  ...
  private
    def unverified(status)
      count!("unverified", request.remote_ip)
      head status
    end
