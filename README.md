...
**Or update from anywhere:** click the version on the flight board. Houston's page checks for a newer release now (**Check now**, or `houston update --check`) and updates to it (**Update to vX.Y.Z**, or `houston update`, which follows it to the end): Mission Control runs that release's installer on the server for you, and the page shows its steps and log as a deploy's page does. It waits until no deploy, restore or backup is running, and holds new ones until it's done. Mission Control restarts on the way, then the board shows the new version (or, if it failed, a red "Update to vX.Y.Z failed" that leads to the log). If the new version doesn't install, the server goes back to the one it ran. The first version with the button has to be installed with the command above.
