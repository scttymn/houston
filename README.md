...
**Or update from anywhere:** in **Settings › Releases**, **Check for updates** asks GitHub now (`houston update --check`), and **Update to vX.Y.Z** updates (`houston update`, which follows it to the end): Mission Control runs that release's installer on the server for you. Each update is listed there, with a log page like a deploy's: its steps and its log. It waits until no deploy, restore or backup is running, and holds new ones until it's done. Mission Control restarts on the way, then the board shows the new version (or, if it failed, a red "Update to vX.Y.Z failed" beside it that leads to the log). If the new version doesn't install, the server goes back to the one it ran. The first version with the button has to be installed with the command above.
