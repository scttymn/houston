...
# Cloudflare API from inside the machine; the token is never printed.
set -euo pipefail
...
step1=$(vm curl -s -o /dev/null -w '%{http_code} %{redirect_url}' -c "$jar" -b "$jar" -X POST "http://$ip:3000/setup" \
  ...
  --data-urlencode "setup[password]=a long test-only password" --data-urlencode "setup[password_confirmation]=a long test-only password")
